...
class IDEASessionScheduledStopEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        self.log_info(message_id=message_id, message=f'handle stop RES Session ID: {idea_session_id} for owner: {idea_session_owner} Force: {force}')
        ...
        if Utils.is_empty(session):
            self.log_error(message_id=message_id, message=f'Trying to stop Invalid RES Session ID: {idea_session_id} and owner: {idea_session_owner}. No OP. Returning.')
            return
