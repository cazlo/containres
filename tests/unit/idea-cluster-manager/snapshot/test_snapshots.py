...
from containres.ideaclustermanager.app.snapshots.snapshots_service import DYNAMODB_TABLES_TO_EXPORT
from tests.unit.ideatestutils import IdeaTestProps
...
def test_apply_snapshot_with_missing_bucket_name_should_fail(context: AppContext):
    """
    create snapshot with missing bucket name should fail
    """
    with pytest.raises(exceptions.SocaException) as exc_info:
...
def test_apply_snapshot_with_invalid_bucket_name_should_fail(context: AppContext):
    """
    create snapshot with invalid bucket name that doesn't match the required regex pattern.
    """
    with pytest.raises(exceptions.SocaException) as exc_info:
        context.snapshots.apply_snapshot(
            snapshot=Snapshot(s3_bucket_name="invalid@bucket_name")
        )
    assert exc_info.value.error_code == errorcodes.INVALID_PARAMS
    assert "s3_bucket_name must match regex" in exc_info.value.message
...
def test_apply_snapshot_with_missing_snapshot_path_should_fail(context: AppContext):
    """
    create snapshot with missing snapshot path should fail
    """
    with pytest.raises(exceptions.SocaException) as exc_info:
...
def test_apply_snapshot_with_invalid_snapshot_path_should_fail(context: AppContext):
    """
    create snapshot with invalid snapshot path that doesn't match the required regex pattern.
    """
    with pytest.raises(exceptions.SocaException) as exc_info:
